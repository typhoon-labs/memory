...
const { mockListActive } = vi.hoisted(() => ({
  mockListActive: vi.fn().mockResolvedValue([]),
}));
...
import { refreshScheduler, stopScheduler } from './sync-scheduler';

/** Helper: set up SyncTargetRepo.listActive() to return targets */
function setupDbTargets(targets: Array<{ id: string; name: string; cronSchedule: string }>) {
  mockListActive.mockResolvedValue(targets);
}
...
describe('refreshScheduler', () => {
  ...
  it('enqueues a "scan" job with { syncTargetId } when cron fires', async () => {
    ...
    expect(mockQueueAdd).toHaveBeenCalledWith(
      'scan',
      { syncTargetId: 'st-42' },
      expect.objectContaining({ jobId: expect.stringMatching(/^[0-9a-f]{8}-[0-9a-f]{4}-/), priority: 3 }),
    );
  });
  ...
  it('queries SyncTargetRepo for active targets', async () => {
    setupDbTargets([]);
    await refreshScheduler();
...
describe('failed job retention', () => {
  ...
  it('sets up retention interval on first refresh', async () => {
    await expect(refreshScheduler()).resolves.not.toThrow();
    stopScheduler();
  });
  ...
  it('retention callback handles errors gracefully', async () => {
    ...
    await expect(vi.advanceTimersByTimeAsync(24 * 60 * 60 * 1000)).resolves.not.toThrow();
  });
});
