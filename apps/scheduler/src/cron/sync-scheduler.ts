...
import { getSyncQueue } from '../infra/queue';
import { getSyncTargetRepo } from '../services';

const log = createAppLogger('scheduler');
const syncTargetRepo = getSyncTargetRepo();
...
export async function refreshScheduler(): Promise<void> {
  ...
  const targets = await syncTargetRepo.listActive();
