...
export class DrizzleMemoryStorage extends MemoryStorage {
  ...
  private async resolveThreadId(externalId: string): Promise<string | null> {
    const [row] = await this.db.select({ id: threads.id }).from(threads).where(eq(threads.externalId, externalId));
    return row?.id ?? null;
  }
    ...
    const rows = await this.db
      ...
      .values(
        args.messages.map((msg) => ({
          ...
          role: msg.role,
          type: msg.type ?? 'text',
          content: stripChunkSources(msg.content as Record<string, unknown>),
