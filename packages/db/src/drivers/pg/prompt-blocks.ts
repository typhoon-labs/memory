...
const driver = createVersionedDriver({
  mainTable: promptBlocks,
  versionTable: promptBlockVersions,
  mainId: promptBlocks.id,
  versionEntityId: promptBlockVersions.blockId,
  versionNumber: promptBlockVersions.versionNumber,
  authorId: promptBlocks.authorId,
  createdAt: promptBlocks.createdAt,
  updatedAt: promptBlocks.updatedAt,
  versionId: promptBlockVersions.id,
  versionCreatedAt: promptBlockVersions.createdAt,
});

export class DrizzlePromptBlocksStorage extends PromptBlocksStorage {
  ...
  async create(input: never) {
    const { promptBlock } = input as { promptBlock: Record<string, unknown> };
    return driver.create(this.db, {
      id: (promptBlock.id as string) ?? crypto.randomUUID(),
      status: (promptBlock.status as string) ?? 'draft',
      activeVersionId: promptBlock.activeVersionId ?? null,
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return driver.listVersions(this.db, data.blockId as string, data as never) as never;
  }
