...
export class DrizzleMCPServersStorage extends MCPServersStorage {
  ...
  constructor(sql: Sql) {
    ...
    this.helper = new VersionedStorageHelper(sql, {
      mainTable: 'mcp_servers',
      versionsTable: 'mcp_server_versions',
      entityIdColumn: 'mcp_server_id',
    });
  }
  ...
  async create(input: never) {
    const data = (input as { mcpServer: Record<string, unknown> }).mcpServer ?? (input as Record<string, unknown>);
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.mcpServerId as string, data as never) as never;
  }
