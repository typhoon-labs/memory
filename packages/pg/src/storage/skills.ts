...
export class DrizzleSkillsStorage extends SkillsStorage {
  ...
  async create(input: never) {
    const data = (input as { skill: Record<string, unknown> }).skill ?? (input as Record<string, unknown>);
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.skillId as string, data as never) as never;
  }
