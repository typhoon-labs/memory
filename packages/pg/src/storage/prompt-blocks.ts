...
export class DrizzlePromptBlocksStorage extends PromptBlocksStorage {
  ...
  constructor(sql: Sql) {
    ...
    this.helper = new VersionedStorageHelper(sql, {
      mainTable: 'prompt_blocks',
      versionsTable: 'prompt_block_versions',
      entityIdColumn: 'block_id',
    });
  }
  ...
  async create(input: never) {
    const data = (input as { promptBlock: Record<string, unknown> }).promptBlock ?? (input as Record<string, unknown>);
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.blockId as string, data as never) as never;
  }
