...
export class DrizzleMCPClientsStorage extends MCPClientsStorage {
  ...
  constructor(sql: Sql) {
    ...
    this.helper = new VersionedStorageHelper(sql, {
      mainTable: 'mcp_clients',
      versionsTable: 'mcp_client_versions',
      entityIdColumn: 'mcp_client_id',
    });
  }
  ...
  async create(input: never) {
    const data = (input as { mcpClient: Record<string, unknown> }).mcpClient ?? (input as Record<string, unknown>);
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.mcpClientId as string, data as never) as never;
  }
