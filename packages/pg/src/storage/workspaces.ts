...
export class DrizzleWorkspacesStorage extends WorkspacesStorage {
  ...
  constructor(sql: Sql) {
    ...
    this.helper = new VersionedStorageHelper(sql, {
      ...
      versionsTable: 'workspace_versions',
      entityIdColumn: 'workspace_id',
    });
  }
  ...
  async create(input: never) {
    const data = (input as { workspace: Record<string, unknown> }).workspace ?? (input as Record<string, unknown>);
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.workspaceId as string, data as never) as never;
  }
