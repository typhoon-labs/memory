...
export class DrizzleAgentsStorage extends AgentsStorage {
  ...
  async create(input: never) {
    ...
    return this.helper.create({
      ...
      active_version_id: agent.activeVersionId ?? null,
      author_id: agent.authorId ?? null,
  ...
  async update(input: never) {
    ...
    return this.helper.update(id, sets) as never;
  }
  ...
  async listVersions(input: never) {
    const data = input as Record<string, unknown>;
    return this.helper.listVersions(data.agentId as string, data as never) as never;
  }
